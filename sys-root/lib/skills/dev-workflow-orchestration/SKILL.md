...
1. 【强制】调用 architecture-review Skill
   ...
   ├─ 判定：✓通过 / △有条件通过 / ✗不通过
   ├─ 重点：快速Checklist评审（方案可落地性）
   │         - PRD可追溯性、接口契约完整性、数据模型正确性
   │         - 多端适配、可测试性
   └─ 时长：约30分钟
...
| Skill | 定位 | 核心问题 | 第一性原理 |
|-------|------|---------|-----------|
| architecture-review | 快速Checklist评审 | "方案能直接开发吗？" | 能落地 |
| technical-design-review | 深度量化评审 | "方案能稳定上线吗？" | 能上线 |
...
**规则**：
- 主功能编号：正整数，如 `F-1`、`F-2`、`F-3`
- 子功能编号：最多2位小数，如 `F-1.1`、`F-1.2`、`F-1.15`
- 子功能编号可选，不带小数点时表示顶层功能如 `F-2`
...
`check_solution.py` 的 `check_prd_alignment()` 方法使用以下正则校验功能ID：
...
- `F-` 前缀（必须）
- `\d+` 主功能编号（必须，至少1位）
- `(?:\.\d{1,2})?` 可选的子功能编号（最多2位小数）

### 与PRD的对应关系

| 场景 | 功能ID格式 | 说明 |
|------|-----------|------|
| PRD功能点 | `F-1` ~ `F-9` | 顶层功能，粒度适中 |
| 设计文档 | `F-1.1` / `F-1.2` | 可细分到子功能 |
| 量化评分 | 自动提取并匹配 | 支持上述所有格式 |

### 历史兼容

旧格式 `FR-1.1` 已废弃，请统一使用 `F-1.1` 格式。
...
7. **功能ID格式**：统一使用 `F-\d+(?:\.\d{1,2})?` 格式，禁止使用旧格式 `FR-`
