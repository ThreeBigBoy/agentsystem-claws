...
产出:
  ...
  - 或: docs/project-prd-changes/复盘报告-YYYYMMDD-最近N天工作内容复盘.md（跨 change-id）
```
...
| 产出物 | 存放路径 | 管理仓库 | 说明 |
|--------|---------|---------|------|
| 复盘报告（框架级） | `agentsystem 内框架级复盘（路径以仓库实际为准，如 docs/retrospective/framework/）[YYYY-QN]/复盘-[主题]-YYYY-MM-DD.md` | **agentsystem** | 框架能力成长复盘 |
| 复盘报告（项目级） | `[业务项目]/docs/retrospective/[YYYY-MM]/复盘-[主题]-YYYY-MM-DD.md` | **业务项目** | 业务项目交付复盘 |
| 复盘报告（change-id 级） | `[业务项目]/docs/project-prd-changes/[change-id]/records/复盘报告-YYYY-MM-DD-[主题].md` | **业务项目** | 单个变更详细复盘 |
| 排查备忘录 | `[业务项目]/docs/project-prd-changes/[change-id]/records/XXX问题排查备忘录.md` | **业务项目** | 问题诊断型复盘 |
| Memory | `agentsystem/memory/{patterns,anti-patterns,preferences,playbooks,reflections}/` | **agentsystem** | 经验沉淀 |

### 复盘报告存放路径决策（分布式管理架构）

```yaml
⚠️ 核心原则: 数据就近存储（Data Locality）
  • 框架级复盘 → 存储在 agentsystem（框架仓库）
  • 项目级/change-id 级复盘 → 存储在对应的业务项目中
  • 不重复存储: agentsystem 不替业务项目管理其复盘

框架级复盘（agentsystem 能力成长）:
  场景: 突破性思维复盘、质量闭环升级、技能体系完善、Memory 机制优化
  管理方: agentsystem（唯一）
  路径: agentsystem 内框架级复盘（路径以仓库实际为准，如 docs/retrospective/framework/）[YYYY-QN]/
...
项目级复盘（业务项目交付）:
  ...
  路径: Proj01ShopifyTheme/docs/retrospective/[YYYY-MM]/
  命名: 复盘-[主题关键词]-YYYY-MM-DD.md
  示例: 复盘-商品详情页优化-2026-03-15.md
  注意: 不存放在 agentsystem 中！

change-id 级复盘（单个变更详细复盘）:
  场景: 具体 change-id 的详细执行分析
  管理方: 各项目自身（业务项目 or agentsystem）
  路径: 
    - 业务项目: Proj01ShopifyTheme/docs/project-prd-changes/[change-id]/records/
    - agentsystem: agentsystem 内变更记录（路径以仓库实际为准，如 docs/project-prd-changes/）[change-id]/records/
  命名: 复盘报告-YYYY-MM-DD-[主题].md
  示例: 
    - 业务项目: 复盘报告-2026-03-15-商品详情页优化.md
    - agentsystem: 复盘报告-2026-03-14-自检脚本超时问题排查.md
```
...
```yaml
❌ 错误示例 1: 把项目级复盘放在框架仓库
  错误路径: agentsystem/docs/retrospective/projects/shopify-theme/
  正确路径: Proj01ShopifyTheme/docs/retrospective/

❌ 错误示例 2: 把业务项目的 change-id 复盘放在框架仓库
  错误路径: agentsystem/docs/project-prd-changes/update-theme-v1.0.2/records/
  正确路径: Proj01ShopifyTheme/docs/project-prd-changes/update-theme-v1.0.2/records/

✓ 正确示例: 框架级复盘放在框架仓库
  正确路径: agentsystem 内框架级复盘（路径以仓库实际为准，如 docs/retrospective/framework/）2026-Q1/
```
...
| 复盘级别 | 存放路径（agentsystem） | 存放路径（业务项目） |
|---------|------------------------------|-------------------|
| **框架级** | `docs/retrospective/framework/[YYYY-QN]/复盘-[主题]-YYYY-MM-DD.md` | 无（框架复盘只在框架仓库） |
| **项目级** | 不适用 | `[项目]/docs/retrospective/[YYYY-MM]/复盘-[主题]-YYYY-MM-DD.md` |
| **change-id级** | `docs/project-prd-changes/[change-id]/records/复盘报告-YYYY-MM-DD-[主题].md` | `[项目]/docs/project-prd-changes/[change-id]/records/复盘报告-YYYY-MM-DD-[主题].md` |
