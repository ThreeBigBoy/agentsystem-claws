...
产出技术方案时，首先问自己：**这份技术方案能回答哪些本质问题？**

如果读者看完后无法回答这些问题，说明方案设计得不够本质。

### 6维度产出框架

| 维度 | 本质问题 | 产出必须回答 | 质量标准 |
|------|----------|-------------|---------|
| **PRD对应性** | 做什么 | 功能点是否全覆盖？有无遗漏或过度实现？ | 功能点与PRD功能点一一对应，有可追溯引用 |
| **接口定义清晰** | 怎么交互 | 模块间如何协作？参数和返回值是什么？ | 有完整接口协议，包括输入/输出/错误码 |
| **技术选型合理** | 用什么做 | 为什么选这个技术栈？不选竞品的原因？ | 有需求分析、有候选方案对比、有决策依据 |
| **数据流设计** | 怎么做 | 数据从哪里来、经过什么处理、存到哪里去？ | 有完整数据流图、有状态机定义 |
| **存储设计** | 数据存哪 | 数据如何持久化？容量如何控制？ | 有存储结构定义、有容量控制策略 |
| **项目结构** | 怎么组织 | 代码如何组织？模块边界是什么？ | 有目录结构说明、有模块划分、有规范遵循 |
...
完成design.md后，用以下问题自检：

| 维度 | 自检问题 | 通过标准 |
|------|---------|---------|
| PRD对应性 | 所有PRD功能点都有对应实现设计？ | 是/否，有遗漏项列出 |
| 接口定义清晰 | 每个接口都有输入/输出/错误码定义？ | 是/否，缺什么列出 |
| 技术选型合理 | 选型有对比分析？有决策依据？ | 是/否，补充什么列出 |
| 数据流设计 | 数据流完整？有状态机定义？ | 是/否，缺什么列出 |
| 存储设计 | 有存储结构？有容量控制？ | 是/否，缺什么列出 |
| 项目结构 | 有目录结构？有模块划分？ | 是/否，缺什么列出 |

### 与评审的对应关系

| 维度 | architecture-review关注点 | technical-design-review关注点 |
|------|-------------------------|----------------------------|
| PRD对应性 | 功能点覆盖、可追溯 | 功能完整性 |
| 接口定义清晰 | 接口契约、数据模型 | 接口健壮性 |
| 技术选型合理 | 与project.md一致 | 技术风险应对 |
| 数据流设计 | 流程图示 | 状态管理完整性 |
| 存储设计 | 异常与边界 | 数据安全措施 |
| 项目结构 | 目录规范 | 可维护性、扩展性 |
...
**技能版本**: v1.1（2026-03-28 升级：添加第一性原理产出标准，与6维度评审框架对齐）
